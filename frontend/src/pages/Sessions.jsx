...
const Sessions = () => {
  ...
  const fetchActiveSession = async () => {
    try {
      setLoading(true);
      const res = await axios.get('http://localhost:5000/api/parking/session/history');
  ...
  const handleEndSession = async () => {
    ...
    try {
      await axios.post('http://localhost:5000/api/parking/session/end', { sessionId: activeSession._id });
      ...
      {!activeSession ? (
        ...
        <div className="bg-slate-800 border border-emerald-500/30 rounded-3xl p-8 shadow-[0_0_50px_rgba(16,185,129,0.1)] relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="w-full bg-slate-900/80 rounded-xl p-4 border border-blue-500/30 text-center text-blue-400 text-sm mt-4">
              Your parking session is active. It will be ended automatically when your vehicle exits the facility via our AI Camera logs.
            </div>
          </div>
        </div>
      )}
