...
const BookSlot = () => {
  ...
  useEffect(() => {
    if (selectedZone) {
      const fetchSlots = async () => {
        setLoading(true);
        try {
          const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/parking/slots/${selectedZone}`);
          setSlots(res.data);
        } catch (err) {
  ...
  const handleBook = async (slotId) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/parking/book`, { slotId });
      // Navigate to Sessions where user can start the active tracker manually
      navigate('/session');
    } catch (err) {
